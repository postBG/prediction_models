...
from log_data.processor import DefaultPreprocessor, ReadyPreProcessor, FeedForwardProcessor, split_data
...
tf.app.flags.DEFINE_integer('date', 10,
                            """테스트에 사용할 날짜""")
tf.app.flags.DEFINE_string('model', 'neural_network',
                           """테스트에 사용할 모델 지정""")
...
def main():
    ...
    data_processor = DefaultPreprocessor()

    batch = data_processor.process(batch)
    train_data, validate_data, test_data = split_data(batch, validation_rate=0)

    x_train, y_train = data_processor.separate_features_and_label(train_data)
